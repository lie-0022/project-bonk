...
- **3스테이지 진행 체인**(검증됨): Stage1(슬라임)→Stage2(고블린)→Stage3(스켈레톤)→Win. Stage1/2는 동일 구조(PlayerSpawn+스폰영역1+게이트1, 보스 사망 시 게이트 자동 개방), Stage3만 봉인+2단보스 특수 구조(게이트는 ArenaPhaseManager가 개방, `_isFinalStage=true`→Win)
- **난이도 배율**(`StageDifficulty`): 맵별 적 HP/데미지/보상 배율. (스켈레톤 데미지 1.0·고블린 1.1로 완화 + 플레이어 피격무적 0.4s·HP150 — 떼 즉사 방지)
...
- **점검 결과**: 보스 차별화 명확(점프/돌진/원거리). ✅ **Stage2(고블린) GamePlay 통합 검증 완료** — PlayerSpawn+스폰영역+게이트 배선 정상, Stage1과 동일 구조의 플레이 가능 스테이지. 진행 체인 Stage1→2→3→Win 게이트 인덱스 모두 정상 연결 확인.
