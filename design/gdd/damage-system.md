...
데미지 시스템은 공격이 피해를 발생시키는 방식을 정의하는 Core 레이어 컴포넌트다. 스킬, 기본 공격, 적의 공격 모두 이 시스템을 통해 `HealthComponent.TakeDamage()`를 호출한다. MVP에서는 단순 고정 피해(flat damage) 방식을 사용하며, 향후 크리티컬·방어력 등 확장 가능한 구조로 설계한다.

## Player Fantasy

스킬을 발동했을 때 적의 체력바가 눈에 띄게 줄어드는 것이 파워 트립의 시각적 핵심이다. 숫자가 크게 튀어나올수록, 적이 빠르게 사라질수록 플레이어는 강해진다는 느낌을 받는다. 데미지 시스템이 이 피드백의 근간이다.
...
2. 피해를 주는 주체(스킬, 기본 공격, 적)는 `DamageDealer.Deal(DamageInfo, HealthComponent target)`을 호출한다
...
- **동시 다중 히트**: 범위 스킬이 같은 프레임에 같은 적을 여러 번 히트하는 경우 → 히트 판정마다 독립적으로 피해 적용 (단, 스킬별로 동일 타겟 히트 쿨다운 설정 가능)
...
| 변수 | 기본값 | 안전 범위 | 설명 |
|------|--------|----------|------|
| 기본 공격 피해 | 10 | 5 ~ 20 | 낮을수록 스킬 의존도 증가 |
