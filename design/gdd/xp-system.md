...
경험치 시스템은 적 처치로 경험치를 획득하고, 임계값 도달 시 레벨업해 스킬 선택 화면을 트리거하는 진행 시스템이다. 레벨이 높아질수록 레벨업에 필요한 XP가 선형으로 증가하며, 10분 런 기준으로 약 8~10회 레벨업이 가능하도록 설계한다.

## Player Fantasy

적을 처치할 때마다 경험치 바가 채워지는 시각적 피드백이 "다음 레벨업까지 조금만 더"를 유도한다. 레벨업 순간 게임이 잠시 멈추고 스킬 선택 화면이 뜨는 그 순간이 런의 핵심 쾌감 포인트다.
...
| LevelingUp | `currentXP >= xpToNextLevel` | 레벨업 처리 → 스킬 선택 이벤트 발생 |
...
10분 런 예상 레벨업 횟수:
  ...
  → 런당 약 7~9회 레벨업 (스킬 선택 7~9회)
```

## Edge Cases

- **연속 레벨업**: 한 번에 다수의 적을 처치해 XP가 두 배 이상 들어오면 `while` 루프로 연속 레벨업 처리. 스킬 선택 화면은 레벨업당 1회씩 순차적으로 표시
