using UnityEngine;

/// <summary>
/// 골드 오브 동작. XPOrb와 동일한 구조로 플레이어가 attractRadius 안에 들어오면 수집된다.
/// </summary>
public class GoldOrb : MonoBehaviour, IPoolable
{
    [SerializeField] private float _attractRadius = 5f;
    [SerializeField] private float _flySpeed = 8f;
    [SerializeField] private float _collectRadius = 0.5f;
    [SerializeField] private float _bobSpeed = 2f;
    [SerializeField] private float _bobHeight = 0.15f;

    private int _goldAmount;
    private Transform _playerTransform;
    private bool _isAttracting;
    private float _bobOffset;
    private Vector3 _spawnPosition;

    /// <summary>GoldSystem이 스폰 후 호출해 골드량과 플레이어 참조를 주입한다.</summary>
    public void Setup(int goldAmount, Transform playerTransform)
    {
        _goldAmount = goldAmount;
        _playerTransform = playerTransform;
        _spawnPosition = transform.position;
        _bobOffset = Random.Range(0f, Mathf.PI * 2f);
    }

    public void OnSpawn()
    {
        _isAttracting = false;
        _goldAmount = 0;
        _playerTransform = null;
    }

    public void OnDespawn() { }

    private void Update()
    {
        if (_playerTransform == null) return;

        if (_isAttracting)
        {
            transform.position = Vector3.MoveTowards(
                transform.position,
                _playerTransform.position,
                _flySpeed * Time.deltaTime);

            if (Vector3.Distance(transform.position, _playerTransform.position) <= _collectRadius)
                Collect();
        }
        else
        {
            float y = _spawnPosition.y + Mathf.Sin(Time.time * _bobSpeed + _bobOffset) * _bobHeight;
            transform.position = new Vector3(_spawnPosition.x, y, _spawnPosition.z);

            float sqrDist = (transform.position - _playerTransform.position).sqrMagnitude;
            if (DropItemEffects.MagnetActive || sqrDist <= _attractRadius * _attractRadius)
                _isAttracting = true;
        }
    }

    private void Collect()
    {
        GoldSystem.Instance.AddGold(_goldAmount);
        PickupPool.Instance.ReturnGoldOrb(gameObject);
    }
}
